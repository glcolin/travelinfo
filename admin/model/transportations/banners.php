<?php
class ModelTransportationsBanners extends Model {
	
	public function getTotalBanners($data = array()){
		$query = $this->db->query("SELECT * from aa_transportations_banners where language_id=1 order by sort asc,update_date desc");
		$rows=$query->rows;
	...
	public function getBanner($id) {
		$query = $this->db->query("SELECT * from aa_transportations_banners where item_id=".$id);
		$rows=$query->rows;
		$result=array();
		foreach($rows as $row){
	...
	// print_r($data);
		...
		foreach($languages as $language){
			if($i==1){
				...
				$this->db->query("update aa_transportations_banners set item_id=".$item_id." where  id=".$item_id);
			}
			else{
				$this->insert_action($data,$language,$item_id);
			}
	...
	public function deleteBanner($data=array()){
		foreach($data['selected'] as $id){
			$query=$this->db->query("select item_id from aa_transportations_banners where id=".$id);
			$row=$query->row;
			$this->db->query("delete from aa_transportations_banners where item_id=".$row['item_id']);
		}
	}
	...
	public function update_banner($data=array()){
		...
		$query=$this->db->query("select item_id from aa_transportations_banners where id=".$data['banner_id']);
		$row=$query->row;
		$item_id=$row['item_id'];
		
		foreach($languages as $language){
			$query2=$this->db->query("select id from aa_transportations_banners where item_id=".$row['item_id']." and language_id=".$language['language_id']);
			if($query2->row){
				$this->db->query("update aa_transportations_banners set 
	...
	public function insert_action($data,$language,$item_id){
		$this->db->query("insert into aa_transportations_banners (item_id,title,link,position,image_url,language_id,create_date,update_date) values ('".
		$item_id."','".
	...
	public function saveSort($data=array()){
		...
		foreach($sort_arr as $key=>$id){
			$query=$this->db->query("update aa_transportations_banners set sort='".$key."' where item_id='".$id."'");
