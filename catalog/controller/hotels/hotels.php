...
class ControllerHotelsHotels extends Controller {
	public function index() {
		...
		$header_banners = $this->model_hotels_hotels->getBanners($data);
		...
		$categorys = $this->model_hotels_hotels->getCategorys($data);
		...
		//items
		$ids=array();
		if(isset($this->session->data['hotels_ids'])){
			$ids = $this->session->data['hotels_ids'];
		}
		$this->data['ids'] = $ids;
		...
		$items = $this->model_hotels_hotels->getItems($data);
		$data=array(
			"category"=>$current_category,
			"search"=>$search,
			"language_id"=>$language
		);
		$items_total = $this->model_hotels_hotels->getItems($data);
		...
		$this->template = 'hotels/hotels.tpl';
