<?php  
class ControllerRestaurantsCompare extends Controller {
	public function index() {
		$this->document->setTitle("America Travel Info");
	
		$this->load->model('restaurants/compare');
		...
		$header_banners = $this->model_restaurants_compare->getBanners($data);
		...
		$categorys = $this->model_restaurants_compare->getCategorys($data);
		...
		$this->session->data['restaurants_ids'] = array();
		
		$data=array(
			"ids" => $ids,
			"language_id"=>$language
		);
		$items = $this->model_restaurants_compare->getItems($data);
		
		$items_result=array();
		foreach($items as $item){
			$item['title_link'] = $item['custom_link']=="yes"?$item['website']:'./index.php?route=restaurants/restaurant&cid='.$item['category'].'&item_id='.$item['item_id'];
			$items_result[] = $item;
		}
	
		$this->data['items'] = $items_result;

		$this->template = 'restaurants/compare.tpl';
