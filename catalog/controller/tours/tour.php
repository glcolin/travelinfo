<?php  
class ControllerToursTour extends Controller {
	public function index() {
		$this->document->setTitle("America Travel Info");
	
		$this->load->model('tours/tour');
		...
		$header_banners = $this->model_tours_tour->getBanners($data);
		...
		//category
		$this->session->data['class_type'] = "tours";
		$this->session->data['class_title'] = "tours";
		$data=array(
			"language_id"=>$language
		);
		$categorys = $this->model_tours_tour->getCategorys($data);
		...
		$this->data['item'] = $this->model_tours_tour->getItem($data);

		$this->template = 'tours/tour.tpl';
